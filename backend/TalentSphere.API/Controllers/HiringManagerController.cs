...
public class HiringManagerController : ApiControllerBase
{
    ...
    [HttpPost("ai/interview-questions")]
    public async Task<ActionResult<IReadOnlyList<GeneratedInterviewQuestionDto>>> GenerateInterviewQuestions([FromBody] GenerateInterviewQuestionsRequest request, CancellationToken ct) =>
        Ok(await _hiringManagerService.GenerateInterviewQuestionsAsync(request, ct));
}
