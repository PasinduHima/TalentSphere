...
public static class DependencyInjection
{
    public static IServiceCollection AddInfrastructure(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddHttpClient<IInterviewQuestionGenerationService, GeminiInterviewQuestionService>(client =>
        {
            client.BaseAddress = new Uri("https://generativelanguage.googleapis.com/");
            // Observed latency against the live API ranges from ~10s to 30s+
            // (and occasionally longer under throttling); 45s gives genuine
            // slow-but-successful calls room without leaving the UI hanging
            // indefinitely — GenerateAsync falls back to template questions
            // on timeout either way, so the request never truly gets "stuck".
