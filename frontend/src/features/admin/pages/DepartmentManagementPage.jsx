...
const DUMMY_ORGANIZATIONS = [
  { id: 'o1', name: 'Acme Corp', industry: 'Technology', website: 'https://acme.com' },
];

const DUMMY_DEPARTMENTS = [
  { id: 'd1', name: 'Engineering', organizationName: 'Acme Corp', userCount: 142, jobCount: 12 },
  { id: 'd2', name: 'Human Resources', organizationName: 'Acme Corp', userCount: 8, jobCount: 1 },
  { id: 'd3', name: 'Data Science', organizationName: 'Acme Corp', userCount: 35, jobCount: 4 },
  { id: 'd4', name: 'Product', organizationName: 'Acme Corp', userCount: 24, jobCount: 3 },
  { id: 'd5', name: 'IT Support', organizationName: 'Acme Corp', userCount: 15, jobCount: 2 },
  { id: 'd6', name: 'Sales', organizationName: 'Acme Corp', userCount: 89, jobCount: 8 },
];
...
export default function DepartmentManagementPage() {
  ...
  const load = () => {
    ...
    Promise.all([adminApi.getDepartments(), adminApi.getOrganizations()])
      .then(([depts, orgs]) => {
        setDepartments(depts?.length ? depts : DUMMY_DEPARTMENTS);
        setOrganizations(orgs?.length ? orgs : DUMMY_ORGANIZATIONS);
      })
      .catch(() => {
  ...
  const handleCreateOrg = async (values) => {
    ...
    try {
      ...
      message.success('Organization created (demo).');
      setOrganizations([...organizations, { ...values, id: 'o' + Date.now() }]);
      setOrgModalOpen(false);
      orgForm.resetFields();
    } finally {
      setSaving(false);
  ...
  const handleCreateDepartment = async (values) => {
    ...
    try {
      ...
      const orgName = organizations.find(o => o.id === values.organizationId)?.name || 'Acme Corp';
      message.success('Department created (demo).');
      setDepartments([...departments, { ...values, id: 'd' + Date.now(), organizationName: orgName, userCount: 0, jobCount: 0 }]);
      setModalOpen(false);
      form.resetFields();
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      message.success('Department deleted (demo).');
