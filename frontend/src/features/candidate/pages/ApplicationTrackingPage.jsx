...
import { Card, Button, Progress, Typography, Space, theme } from 'antd';
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <Card bordered={false} style={{ borderLeft: `4px solid ${token.colorPrimary}`, boxShadow: '0 1px 2px 0 rgba(0,0,0,0.05)', borderRadius: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <Progress type="dashboard" percent={75} format={() => mockApplications.length} size={64} strokeColor={token.colorPrimary} />
          <div>
            <Title level={5} style={{ margin: 0 }}>Active Pipeline</Title>
            <Text type="secondary">You have {mockApplications.length} applications in progress</Text>
          </div>
        </div>
      </Card>
              ...
              <div style={{ padding: '24px', borderBottom: `1px solid ${token.colorBorder}`, display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                <div style={{ display: 'flex', gap: '16px' }}>
                  ...
                  <div>
                    ...
                    <Space size={16} wrap>
                      <Text type="secondary" style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '14px' }}><MapPin size={16} /> Remote</Text>
                      <Text type="secondary" style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '14px' }}><Clock size={16} /> Applied on {new Date(app.appliedAt).toLocaleDateString()}</Text>
                    </Space>
                  </div>
                </div>
              ...
              <div style={{ padding: '24px', backgroundColor: '#f8fafc' }}>
                ...
                <div style={{ marginTop: '24px', display: 'flex', alignItems: 'flex-start', gap: '12px', backgroundColor: '#fff', padding: '16px', borderRadius: '8px', border: `1px solid ${token.colorBorder}`, boxShadow: '0 1px 2px 0 rgba(0,0,0,0.05)' }}>
                  <div style={{ marginTop: '4px' }}>
                    {currentStepIndex === 2 ? (
                      <div style={{ height: '8px', width: '8px', borderRadius: '50%', backgroundColor: '#f59e0b', boxShadow: '0 0 0 4px #fef3c7' }}></div>
                    ) : (
