          ...
          <Col xs={24} md={16}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <Card title="Skills & Expertise" bordered={false} style={{ boxShadow: '0 1px 2px 0 rgba(0,0,0,0.05)', borderRadius: '12px', border: `1px solid ${token.colorBorder}` }} headStyle={{ borderBottom: `1px solid ${token.colorBorder}` }}>
                {/* Plain div, not a <form>: this Card lives inside the page's
                    outer antd <Form>, and nested <form> elements are invalid
                    HTML — the browser routes the submit event to the outer
                    form instead, so "Add" would trigger Save Changes rather
                    than adding a skill. Enter-to-submit is handled via
                    onPressEnter instead. */}
                <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                  ...
                  <Button type="primary" size="large" onClick={handleAddSkill} disabled={!newSkill.trim()}>Add</Button>
                </div>
